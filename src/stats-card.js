...
async function fetchStats(id) {
  let headers = {
    Cookie: `__client_id=${process.env.cid};_uid=${process.env.uid}`,
  ...
  const passed = res.data.data.passedProblemsCount;

  stats.name = user.name;
  stats.color = user.color;
  stats.ccfLevel = user.ccfLevel;
  stats.passed = Array.from({ length: 9 }, () => 0);
  ...
  const practiceRes = await axios.get(
    `https://www.luogu.com.cn/user/${id}/practice`,
    {
      headers,
    },
  );
  ...
  stats.ranking = res.data.data.user.ranking;
